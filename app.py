...
def extract_player_data(player_id):
    ...
    try:
        ...
        for format_key, format_info in formats.items():
            try:
                # Try the summary types that Cricinfo exposes in order of usefulness.
                career_data = None
                for summary_type in ("allround", "batting", "bowling"):
                    candidate_data = player.career_summary(
                        query_params={
                            "class": format_info["class"],
                            "type": summary_type,
                        }
                    )

                    if candidate_data is not None and not candidate_data.empty:
                        career_data = candidate_data
                        print(
                            f"[OK] {format_info['name']}: using {summary_type} summary with {len(candidate_data)} records"
                        )
                        break
                ...
                if career_data is not None and not career_data.empty:
                    ...
                    print(f"[WARN] No data for {format_info['name']}")

            except Exception as e:
                print(f"[ERR] Error in {format_info['name']}: {str(e)}")
                formats_data[format_key] = create_empty_format_data()
...
def process_format_data(df):
    ...
    for _, row in df.iterrows():
        ...
        # Keep only opposition rows (e.g., "v Australia") for team-wise stats.
        if not grouping.lower().startswith("v "):
            continue

        team_name = grouping[2:].strip()
